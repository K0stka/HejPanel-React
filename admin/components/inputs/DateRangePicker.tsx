...
const PRESETS: Preset[] = [
    { name: "today", label: "Dnes" },
    { name: "tomorrow", label: "Zítra" },
    { name: "next7", label: "Příštích 7 dní" },
    { name: "next14", label: "Příštích 14 days" },
    { name: "thisWeek", label: "Tento týden" },
                    ...
                    <div className="text-right">
                        ...
                        {rangeCompare != null && (
                            <div className="-mt-1 text-xs opacity-60">
                                <>
                                    vs.{" "}
                                    {formatDate(rangeCompare.from, locale.code)}
...
export const DateRangePicker: FC<DateRangePickerProps> & {
    ...
    const setRange = (range: DateRange): void => {
        ...
        if (minDate && range.from < minDate) {
            range.from = minDate;
            if (range.to < minDate) range.to = minDate;
        }

        if (maxDate && range.to > maxDate) {
            range.to = maxDate;
            if (range.from > maxDate) range.from = maxDate;
        }
    ...
    const [rangeCompare, setRangeCompareState] = useState<
        DateRange | undefined
    >(
        initialCompareFrom
            ? {
    ...
    const setRangeCompare = (rangeCompare: DateRange | undefined): void => {
        if (!rangeCompare) {
            setRangeCompareState(undefined);
            return;
        }

        rangeCompare.to ??= rangeCompare.from;

        if (minDate && rangeCompare.from < minDate) {
            rangeCompare.from = minDate;
            if (rangeCompare.to < minDate) rangeCompare.to = minDate;
        }

        if (maxDate && rangeCompare.to > maxDate) {
            rangeCompare.to = maxDate;
            if (rangeCompare.from > maxDate) rangeCompare.from = maxDate;
        }
